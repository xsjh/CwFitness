...
export async function POST(request: Request) {
  ...
  try {
    await prisma.$transaction(async (tx) => {
      ...
      await tx.workoutPlan.createMany({ data: backup.plans.map((item) => ({ id: item.id as string, userId: session.user.id, name: item.name as string, version: item.version as number, archivedAt: nullableDate(item.archivedAt), createdAt: asDate(item.createdAt), updatedAt: asDate(item.updatedAt) })) });
