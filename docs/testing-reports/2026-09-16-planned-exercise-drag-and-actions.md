...
> **已被取代（2026-09-17）**：本文描述的手写指针拖拽 / 网格量化实现因「卡片不跟手」已整体删除，
> 改用 dnd-kit。当前实现见 `2026-09-17-planned-exercise-drag-dndkit-migration.md`。
> 本文作为该次迭代的事实快照保留 —— 其中「测试 stub 的网格原点恰好掩盖了 bug」一节仍有参考价值。
